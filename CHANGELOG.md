...
- Open a session in a terminal (hover → **⎘ Open**), which resumes it at its working directory via a configurable command (`claude --resume ${sessionId}` by default). The directory is the session's launch cwd (first seen), so mid-session `cd`s don't break it; a stale directory falls back gracefully with a warning. The terminal is named after the session's title by default (`agentPulse.terminalName`: `title` / `hash` / `auto`, where `auto` lets Claude Code drive the tab title live).
